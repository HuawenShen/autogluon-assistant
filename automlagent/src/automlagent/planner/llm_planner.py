...
class LLMPlanner:
    ...
    def __call__(
        ...
        prompt = self._build_evaluation_prompt(stdout=stdout, stderr=stderr, python_code=python_code, task_prompt=task_prompt, data_prompt=data_prompt)
    ...
    def _build_evaluation_prompt(self, stdout: str, stderr: str, python_code: str, task_prompt: str, data_prompt: str) -> str:
        """
...
Even if the code executed without throwing errors, it might still have issues with logic or not meet all requirements."""
    ...
    def _parse_evaluation_response(self, response: Dict) -> Tuple[str, str, Optional[str]]:
        """
        ...
        if "EXPLANATION:" in content:
            explanation_parts = content.split("EXPLANATION:")[1].split("ERROR_SUMMARY:" if "ERROR_SUMMARY:" in content else "\n\n")
