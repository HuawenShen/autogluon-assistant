...
def generate_initial_prompts(
        ...
        output_folder,
        max_chars_per_file,):
    data_prompt = generate_data_prompt(input_data_folder=input_data_folder, max_chars_per_file=max_chars_per_file)
    task_prompt, tutorials_sub_folder = generate_task_prompt(data_prompt=data_prompt, output_folder=output_folder)  # use LLM to select a task prompt from tabular/automm/timeseries
...
class PromptGenerator:
    ...
    def __init__(self, input_data_folder, tutorials_folder, output_folder, max_chars_per_file=100):
    ...
    def user_prompt(self,):
        if self.time_step >= 0:
            ...
            logger.warning("No user prompt because the prompt generator is not stepped yet.")
            return ""
    ...
    def error_prompt(self,):
        if self.time_step >= 0:
            ...
            logger.warning("No error prompt because the prompt generator is not stepped yet.")
            return ""
        
    @property
    def tutorial_prompt(self,):
        if self.time_step >= 0:
            return self.tutorial_prompts[self.time_step]
        else:
            logger.warning("No tutorial prompt because the prompt generator is not stepped yet.")
            return ""

    def step(self, user_inputs=None, error_message=None,):
        user_prompt = generate_user_prompt(user_inputs)
        error_prompt = generate_user_prompt(error_message)
        tutorial_prompt = generate_tutorial_prompt(task_prompt=self.task_prompt,data_prompt=self.data_prompt,user_prompt=user_prompt,error_prompt=error_prompt)
    ...
    def get_iterative_prompt(self,):
        assert self.time_step >= 0, f"run PromptGenerator.step(user_inputs, error_message) before get the prompt"
        prompt = ""
        if self.time_step == 0:
